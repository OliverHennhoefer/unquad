...
class TestDatasetDownload(unittest.TestCase):
    ...
    def test_bytesio_stream_reuse(self):
        ...
        self.assertEqual(len(data1), len(test_data), "Stream 1 should contain full data")
        self.assertEqual(len(data2), len(test_data), "Stream 2 should contain full data")
        self.assertEqual(len(data3), len(test_data), "Stream 3 should contain full data")
        ...
        self.assertEqual(len(load._DATASET_CACHE["reuse_test.parquet.gz"]), len(test_data))
    ...
    def test_download_caching_behavior(self):
        ...
        with mock.patch('unquad.utils.data.load.urlopen') as mock_urlopen:
            ...
            with mock.patch('unquad.utils.data.load.Request'):
                # First call should trigger download
                stream1 = load._download_dataset("cache_test.parquet.gz", show_progress=False)
                self.assertEqual(mock_urlopen.call_count, 1, "First call should trigger download")
                
                # Second call should use cache (no additional download)
                stream2 = load._download_dataset("cache_test.parquet.gz", show_progress=False)
                self.assertEqual(mock_urlopen.call_count, 1, "Second call should not trigger download")
                
                # Third call should also use cache
                stream3 = load._download_dataset("cache_test.parquet.gz", show_progress=False)
                self.assertEqual(mock_urlopen.call_count, 1, "Third call should not trigger download")
