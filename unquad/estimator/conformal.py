...
class ConformalEstimator:
    ...
    def fit(self, x: Union[pd.DataFrame, np.ndarray]) -> None:
        ...
        if self.method in [Strategy.SPLIT_CONFORMAL]:
        ...
        elif self.method in [Strategy.JACKKNIFE, Strategy.JACKKNIFE_PLUS]:
            len_x = np.shape(x)[0]
            folds = KFold(n_splits=len_x, shuffle=True, random_state=self.seed)

        elif self.method in [Strategy.CV, Strategy.CV_PLUS]:
            folds = KFold(
            ...
            Strategy.JACKKNIFE_AFTER_BOOTSTRAP,
            Strategy.JACKKNIFE_PLUS_AFTER_BOOTSTRAP,
        ]:
    ...
    @staticmethod
    def _sanity_check(method: Strategy, split: SplitConfiguration, alpha: float):
        if not (0.0 < alpha <= 1.0):
            raise ValueError("Parameter 'alpha' should be in range (0, 1].")

        if (
            method in [Strategy.JACKKNIFE, Strategy.JACKKNIFE_PLUS]
        ...
        if (method in [Strategy.CV, Strategy.CV_PLUS]) and split.n_params != 1:
            raise ValueError(
