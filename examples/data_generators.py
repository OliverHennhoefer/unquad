...
def demonstrate_data_loading():
    ...
    print(f"Shuttle dataset: {df_shuttle.shape} (features: {df_shuttle.columns.tolist()[:5]}...)")
    print(f"Class distribution: {df_shuttle['Class'].value_counts().to_dict()}")
    
    # Load with experimental setup
    x_train, x_test, y_test = load_shuttle(setup=True)
    print(f"Setup split - Train: {x_train.shape}, Test: {x_test.shape}, Test anomalies: {y_test.sum()}")
    
    # Show memory cache info
    cache_info = get_memory_cache_info()
    print(f"Memory cache: {len(cache_info['datasets'])} datasets, {cache_info['total_size_mb']} MB")
    print()
...
def demonstrate_batch_generation():
    ...
    for i, (x_batch, y_batch) in enumerate(batch_gen.generate(n_batches=3)):
        ...
        print(f"Batch {i+1}: {x_batch.shape}, Anomalies: {batch_anomalies} ({batch_anomalies/len(x_batch)*100:.1f}%)")
...
def demonstrate_online_streaming():
    ...
    for i, (x_instance, y_label) in enumerate(online_gen.generate_stream(n_instances=n_instances)):
        actual_anomalies += y_label
        feature_means.append(x_instance.iloc[0].mean())
        
        if i < 10:  # Show first 10 instances
            print(f"Instance {i+1}: shape={x_instance.shape}, label={y_label}, mean_features={feature_means[-1]:.3f}")
        elif i == 10:
            print("...")
    
    print(f"Actual anomalies: {actual_anomalies} (matches prediction: {actual_anomalies == predicted_anomalies})")
    print()
...
def demonstrate_advanced_scenarios():
    ...
    for batch_size, anomaly_prop, description in scenarios:
        try:
            x_train, batch_gen = create_batch_generator(
                df, batch_size=batch_size, anomaly_proportion=anomaly_prop, random_state=42
            )
            ...
            print(f"{description}: {anomaly_count} anomalies in batch of {len(x_batch)}")
